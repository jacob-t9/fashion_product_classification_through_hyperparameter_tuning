...
In this dataset, each product is identified by an ID such as 42431, and there is a map of all the product metadata in a CSV file. This CSV file contains the product ID, gender, primary category, secondary category, type of clothing, colour, season, year, usage, and product display name. 

For this project, I will be using the images as the inputs for the model and the secondary category as the outputs for the model.  
...
The hyperparameters I chose to optimise included learning rate, momentum, weight decay, and optimiser type. I used Bayesian optimisation through scikit-optimize to fine-tune these parameters within specified ranges. 
